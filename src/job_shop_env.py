...
class JobShopEnv:
    def __init__(self, orders: List[Order], resources: List[Resource],
        ...
        self.gamma = gamma  # для потенциального shaping (сейчас не используется)
    ...
    def step(self, op_id: str) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        # Продвижение времени до ближайшего освобождения
        ...
        # Награда (только финальная, shaping отключён)
        reward = 0.0
        if self.done:
            total_tardiness = 0.0
            for order in self.orders:
    ...
    def _advance_until_resource_free(self):
        """
        Продвигает время до тех пор, пока хотя бы один ресурс не освободится,
        или все операции не завершатся.
        """
        while True:
            ...
            self._update_ready_status()
            # Если появились готовые операции, выходим, чтобы агент мог выбрать
            if any(self.resource_remaining[rid] == 0.0 and self.resource_current_op[rid] is None and
                ...
                break
            # если всё ещё никто не свободен с готовыми операциями, продолжаем ждать
    ...
    def get_ready_ops_list(self) -> List[Dict]:
        ...
        for order in self.orders:
            for op in order.ops:
                if self.op_status[op.id] == 'ready' and self.resource_remaining[op.resource_id] == 0.0:
    ...
    def _get_remaining_time(self, op: Operation, current_time: float) -> float:
        if self.predictor and hasattr(self.predictor, 'trained') and self.predictor.trained:
            ...
            return self.predictor.predict(op, self.resources[op.resource_id], actual_dt)
