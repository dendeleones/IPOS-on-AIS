...
# ---------- Светлая тема для окна обучения ----------
STYLE_SHEET = """
QMainWindow {
...
class TeacherTaskCard(QFrame):
    def __init__(self, op_id, text, parent=None):
        ...
        self.setStyleSheet("TeacherTaskCard { background-color: #ffffff; border: 1px solid #aaaaaa; border-radius: 8px; }")
        self.setMinimumSize(200, 80)
        self.setMaximumSize(200, 80)
        layout = QVBoxLayout(self)
        label = QLabel(text)
        label.setWordWrap(True)
        label.setStyleSheet("color: #1e1e2e; font-weight: bold; background: transparent;")
...
class TeacherDropZone(QWidget):
    ...
    def __init__(self, resource_id, resource_name, parent=None):
        ...
        layout.addWidget(label)
        self.setStyleSheet("TeacherDropZone { border: 2px dashed #4caf50; background-color: #e8f5e9; border-radius: 8px; }")
...
class TeacherWindow(QMainWindow):
    ...
    def rebuild_dashboard(self):
        ...
        for col, title in enumerate(headers):
            ...
            lbl.setStyleSheet("font-weight: bold; color: #1e1e2e; background: transparent;")
